...
app.get("/:code", async (request, reply) => {
	...
	const result =
		await sql /*sql*/`SELECT id,original_url FROM short_links WHERE short_links.code=${code}`;
...
app.get("/api/links/", async () => {
	const result =
		await sql /*sql*/`SELECT * FROM short_links ORDER BY created_at DESC`;
	return result;
});
...
app.post("/api/links/", async (request, reply) => {
	...
	try {
		const result =
			await sql /*sql*/`INSERT INTO short_links(code,original_url)VALUES(${code},${url}) RETURNING id`;
